...
import static org.apache.flume.channel.kafka.KafkaChannelConfiguration.EVENT_MAX_RETRY_COUNT;
...
import static org.apache.flume.channel.kafka.KafkaChannelConfiguration.RETRY_COUNT;
...
public class KafkaChannel extends BasicChannelSemantics {
  ...
  private Integer eventMaxRetryCount;
  ...
  public void configure(Context ctx) {
    ...
    eventMaxRetryCount = ctx.getInteger(EVENT_MAX_RETRY_COUNT, 10);
...
//    }
  ...
  private class KafkaTransaction extends BasicTransactionSemantics {
    ...
    protected Event doTake() throws InterruptedException {
      logger.debug("Starting event take");
      ...
      if (!consumerAndRecords.get().failedEvents.isEmpty()) {
        ...
        if (logger.isTraceEnabled()) {
          logger.debug("Assignment during take: {}",
        ...
        try {
          ...
          if (consumerAndRecords.get().recordIterator.hasNext()) {
            ...
            e.getHeaders().computeIfAbsent(RETRY_COUNT, k -> String.valueOf(0));
    ...
    protected void doCommit() throws InterruptedException {
      ...
      if (type.equals(TransactionType.PUT)) {
        ...
        if (consumerAndRecords.get().failedEvents.isEmpty() && eventTaken) {
          logger.debug("About to commit batch");
          long startTime = System.nanoTime();
    ...
    protected void doRollback() throws InterruptedException {
      ...
      if (type.equals(TransactionType.PUT)) {
        ...
        for (Event event : events.get()) {
          int count = Integer.valueOf(event.getHeaders().get(RETRY_COUNT));
          if (count <= eventMaxRetryCount) {
            count++;
            event.getHeaders().put(RETRY_COUNT, String.valueOf(count));
            consumerAndRecords.get().failedEvents.add(event);
            counter.addToFailedEventsCounter(1L);
          } else {
            logger.error("dropping the event, {} retried more than max retry count {}", count, event.getBody());
          }
        }
        events.get().clear();
      }
  ...
  private class ConsumerAndRecords {
    ...
    private void poll() {
      logger.debug("Polling with timeout: {}ms channel-{}", pollTimeout, getName());
      try {
        records = consumer.poll(pollTimeout);
        recordIterator = records.iterator();
        logger.debug("{} returned {} records from last poll", getName(), records.count());
      } catch (WakeupException e) {
        logger.debug("Consumer woken up for channel {}.", getName());
      }
    }
    ...
    private void commitOffsets() {
      try {
        ...
        logger.debug("About to clear offsets map.");
        offsets.clear();
      }
    }
    ...
    private void saveOffsets(TopicPartition tp, OffsetAndMetadata oam) {
      offsets.put(tp, oam);
      if (logger.isTraceEnabled()) {
        logger.debug(getOffsetMapString());
      }
    }
  }
}
...
class ChannelCallback implements Callback {
  ...
  public void onCompletion(RecordMetadata metadata, Exception exception) {
    if (exception != null) {
      log.debug("Error sending message to Kafka due to " + exception.getMessage());
    }
