...
import static org.apache.flume.sink.elasticsearch.ElasticSearchSinkConstants.SERVER_URL;
...
public class ElasticSearchSink extends AbstractSink implements Configurable {
  ...
  private ESIndexRequestBuilder indexRequestFactory;
  ...
  public void configure(Context context) {
    ...
    if (StringUtils.isNotBlank(context.getString(SERVER_URL))) {
      serverAddresses = context.getString(SERVER_URL).split(",");
    }
    ...
    try {
      ...
      if (serializer instanceof ESIndexRequestBuilder) {
        indexRequestFactory = (ESIndexRequestBuilder) serializer;
  ...
  public Status process() throws EventDeliveryException {
    logger.debug("ElasticSearchSink processing...");
    org.elasticsearch.client.transport.TransportClient c;
    ...
    try {
      ...
      for (count = 0; count < batchSize; ++count) {
        ...
        client.addEvent(event, indexNameBuilder, realIndexType, -1);
      }
