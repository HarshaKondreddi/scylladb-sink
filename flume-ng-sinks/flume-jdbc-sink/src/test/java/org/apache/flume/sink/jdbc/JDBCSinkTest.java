...
/**
 * Created by shahbaz.khalid on 5/18/18.
 */
...
public class JDBCSinkTest {
  ...
  public void mappedWithH2() throws Exception {
    ...
    conn.prepareStatement("CREATE TABLE public.test (myInteger INTEGER, myString VARCHAR, " +
            "myId BIGINT AUTO_INCREMENT PRIMARY KEY);").execute();
    conn.commit();
    conn.close();
  ...
  public void mappedWithDerby() throws Exception {
    ...
    conn.prepareStatement("CREATE TABLE TEST (myInteger INT, myString VARCHAR(255), \n" +
            "\tmyId INT NOT NULL GENERATED ALWAYS AS IDENTITY \n" +
  ...
  public void templateWithH2() throws Exception {
    ...
    ctx.put("sqlDialect", "H2");
    ctx.put("BATCH_SIZE", "1");
    ctx.put("sql", "INSERT INTO \"PUBLIC\".\"TEST\" (\"MYINTEGER\", \"MYSTRING\") VALUES \n" +
            "\t(cast(${header.myInteger:int} as integer), " +
            "cast(${header.myString:varchar} as varchar))");
    ...
    ResultSet rs = conn
            .prepareStatement("SELECT count(*) AS count FROM public.test").executeQuery();
    rs.next();
...
//        }
  ...
  public void templateWithPostgres() throws Exception {
    ...
    Connection connTruncate = DriverManager
            .getConnection(
                    "jdbc:postgresql://10.200.0.126:5432/test_erige?user=postgres&PASSWORD=");
    connTruncate.prepareStatement("TRUNCATE tc40;").execute();
    //conn.prepareStatement("CREATE TABLE tc40 (myInteger INTEGER, myString VARCHAR,
    // + myId BIGINT AUTO_INCREMENT "
    //        + "PRIMARY KEY);").execute();
    //connTruncate.commit();
    connTruncate.close();
